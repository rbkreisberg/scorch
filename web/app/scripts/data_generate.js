...
return function(options) {
	...
	var fg = {	
		options : {},
		clinical_labels : ['CLIN','SAMP'],
		genomic_labels : ['GEXP','METH','CNVR','MIRN','GNAB'],
		probability_odd_chr_label : 0.05,
		chrom_attr : {
	    '1' : { "length": 247249719 },
	    '2' : { "length": 242951149 },     
	    '3' : { "length": 199501827 },
	...
	fg.chrom_keys = _.keys(fg.chrom_attr);
	_.defaults(fg.options,options,defaults);
	...
	_.extend(fg, {	
		...
		generateFeature : function() {
	        ...
	        var in1 = Math.random()*(this.chrom_keys.length) >>> 0;
	        //put in unmapped chromsomes sometimes
	        var chr1 = this.chrom_keys[in1] + ['_random',''][+(Math.random() > this.probability_odd_chr_label)];
